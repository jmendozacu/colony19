...
class AW_Catalogpermissions_Helper_Data extends Mage_Core_Helper_Abstract
{
     ...
     * MSS integration
     */
    const MSS_NAME = 'AW_Marketsuite';
    const MSS_VERSION = '2.1.1';
    /**
     * MSS-related attributes
     *
     * All of them are simple INT attributes which may contain MSS segment ID or NULL.
     * Zero value has the same meaning as NULL or not set.
     */
    const MSS_INFO = 'aw_cp_mss_info';
    const MSS_DISABLE_PRODUCT = 'aw_cp_mss_disable_product';
    const MSS_DISABLE_PRICE = 'aw_cp_mss_disable_price';
    const MSS_DISABLE_CATEGORY = 'aw_cp_mss_disable_category';
    /**
     * MSS mode
     *
     * AND - require both CP customer group and MSS segment conditions to be true
     * XOR - (yea I know this has different meaning) disable CP customer group check if MSS segment specified
     */
    const MSS_MODE_AND = 0;
    const MSS_MODE_XOR = 1;
    /**
     * Temporarily let's set MSS mode here by constant.
     * It may become a store- or product-specific setting eventually.
     * Don't forget to reflect this logic in product settings (radio buttons for XOR mode for example).
     */
    const MSS_MODE = self::MSS_MODE_XOR;
     ...
     * Returns true if any check succeeds, false if all checks fail or when the product is null.
     *
    ...
    public static function checkVisibility($Product, $flag = true, $mode = array('Price', 'Product'))
    {
        ...
        foreach ($mode as $mod) {
            ...
            $restrictedCustomers = self::getMssRestrictedCustomers($Product, $mod);
            if (self::MSS_MODE == self::MSS_MODE_XOR) {
                $isRestricted =
                    $restrictedCustomers === false
                    && in_array($customerGroupId, $restrictedGroups)
                    || is_array($restrictedCustomers)
                    && in_array(self::getCustomerId(), $restrictedCustomers)
                ;
            }
            else {
                $isRestricted =
    ...
    /**
     * Get customer IDs restricted by MSS rule as plain array.
     * If no customers satisfy the rule, returns empty array.
     * If the rule is disabled, not set or not found, returns false.
     *
     * @param Mage_Catalog_Model_Product $product
     * @param string $mode "Product" or "Price"
     * @return array|bool
     */
    public static function getMssRestrictedCustomers($product, $mode) {
        if (self::isMssEnabled()) {
            $method = "getAwCpMssDisable" . $mode;
            $productRule = $product->{$method}();
            if (
                $productRule
                && Mage::getModel('marketsuite/api')->isRuleActive($productRule)
            ) {
                return Mage::getModel('marketsuite/api')->getCustomerIndexByRuleId($productRule);
            }
        }
        return false;
    }
    ...
    public static function getRestrictedGroupForProduct($product, $method)
    {
        ...
        if ($product->getData('type_id') === Mage_Catalog_Model_Product_Type_Grouped::TYPE_CODE
            && $method === 'getAwCpDisablePrice'
        ) {
        ...
        if ($restrictedGroups) {
            ...
            return array_unique(array_merge($extraRestrictedGroups, $restrictedGroups));
        }
    ...
    public static function setVisibility($product, $flag = true)
    {
        if ($flag) {
            ...
            $product->getStockItem()->setIsInStock(false);
            $product->setAWDisableOutOfStock(true);
            return $product;
        }
    }
    ...
    public static function prepareRewrites()
    {
        ...
        if (Mage::getStoreConfig('advanced/modules_disable_output/AW_Catalogpermissions')) {
            Mage::getConfig()->setNode(
                'global/blocks/catalog/rewrite/seo_sitemap_tree_category',
                'Mage_Catalog_Block_Seo_Sitemap_Tree_Category', true
            );
            Mage::getConfig()->setNode(
                'global/blocks/catalog/rewrite/seo_sitemap_tree_pager', 'Mage_Catalog_Block_Seo_Sitemap_Tree_Pager',
                true
            );
        }
    }
        ...
        if ($product) {
            if (self::checkVisibility($product, true)) {
                ...
                Mage::getSingleton('checkout/session')->addNotice(
                    Mage::helper('catalogpermissions')->__("%s is not for sale", $product->getName())
                );
                unset($product);
            }
        ...
        if (is_array($superGroup)) {
            if (count($superGroup) > 0) {
                foreach ($superGroup as $productId) {
                    ...
                    $subProduct = $product->getTypeInstance(true)->getProductByAttributes(
                        array(key($superGroup) => $productId), $product
                    );
                    ...
                    if ($subProduct) {
                        if (self::checkVisibility($subProduct, true)) {
                            ...
                            Mage::getSingleton('checkout/session')->addNotice(
                                Mage::helper('catalogpermissions')->__("%s is not for sale", $subProduct->getName())
                            );
                        }
                    }
                }
    ...
    public static function checkRecentOrdersAvailability($hiddenProducts = array())
    {
        ...
        foreach ($itemsCollection as $item) {
            $product = Mage::getModel('catalog/product')->setStoreId(Mage::app()->getStore()->getId())->load(
                $item->getProductId()
            );
            if ($product) {
                if (self::checkVisibility($product, true)) {
                    ...
                    Mage::getSingleton('checkout/session')->addNotice(
                        Mage::helper('catalogpermissions')->__("%s is not for sale", $item->getName())
                    );
                }
    ...
    {
        $customer = Mage::getSingleton('customer/session');
        return $customer->isLoggedIn() ? $customer->getCustomer()->getId() : null;
    }

    /**
     * MSS extension and version check
     *
     * @return bool
     */
    public static function isMssInstalled() {
        $MssVersion = Mage::getConfig()->getModuleConfig(self::MSS_NAME)->version;
        if (version_compare($MssVersion, self::MSS_VERSION) >= 0) {
            return true;
        }
        return false;
    }
    ...
    /**
     * Is MSS module enabled check
     *
     * @return bool
     */
    public static function isMssEnabled() {
        if (
            self::isMssInstalled()
            && !Mage::getStoreConfig('advanced/modules_disable_output/'.self::MSS_NAME)
        ) {
            return true;
        }
        return false;
    ...
    public static function isMagentoEE()
    {
