...
class AW_Catalogpermissions_Model_Observer extends AW_Catalogpermissions_Model_Observer_Abstract
{
    ...
    public function controllerActionPredispatch()
    {
        ...
        if (Mage::helper('catalogpermissions')->isMagentoEE()) {
            $node = Mage::getConfig()->getNode('global/models/enterprise_search_resource/rewrite');
            $dnode = Mage::getConfig()->getNode('global/models/enterprise_search_resource/drewrite/collection');
    ...
    public function abstractCollectionBeforeLoad($event)
    {
        ...
        if ($collection instanceof Mage_Wishlist_Model_Mysql4_Item_Collection
            ...
            Mage::helper('catalogpermissions/connection')->addDisabledAttrToFilter(
                $event->getData('collection'), 'product_id', array('noPriceFilter' => true)
            );
        }
        /**
    ...
    public function eavCollectionBeforeLoad($event)
    {
        if (!parent::_validateProcess()) {
            return;
        }
        ...
        if ($collection instanceof Mage_Catalog_Model_Resource_Product_Type_Configurable_Product_Collection
            || $collection instanceof Mage_Catalog_Model_Resource_Eav_Mysql4_Product_Type_Configurable_Product_Collection
        ) {
            $disabled = Mage::registry(AW_Catalogpermissions_Helper_Data::DISABLED_PRICE_PROD_SCOPE);
            $disabled || $disabled = array();
            if (!empty($disabled)) {
                $collection->getSelect()
                    ->where("e.entity_id NOT IN (?)", $disabled);
            }
        }
    }
    ...
    public function blockAbstractToHtmlBefore($event)
    {
        ...
        if ($product = $event->getData('block')->getProduct()) {
            ...
            if (
                $event->getData('block') instanceOf Mage_Catalog_Block_Product_Price
                || $event->getData('block') instanceOf Mage_Wishlist_Block_Render_Item_Price
            ) {
                $disabled = Mage::registry(AW_Catalogpermissions_Helper_Data::DISABLED_PRICE_PROD_SCOPE);
                $disabled || $disabled = array();
                $isDisabledPrice = true;
                if (
                    $product->getTypeId() === Mage_Catalog_Model_Product_Type_Grouped::TYPE_CODE
                ) {
                    $prices = array();
                    ...
                    if (!empty($prices)) {
                        $product->setData('minimal_price', min($prices));
                    } elseif (!in_array($product->getId(), $disabled)) {
                        $disabled[] = $product->getId();
                    }
                }
                if (
                    ...
                    foreach ($product->getTypeInstance()->getUsedProductIds($product) as $childId) {
                        if (!in_array($childId, $disabled)) {
                            $disableParent = false;
                        }
                    }
                    if ($disableParent && !in_array($product->getId(), $disabled)) {
                        $disabled[] = $product->getId();
                    }
                }
    ...
    public function blockAbstractToHtmlAfter($event)
    {
        ...
        $product = $event->getData('block')->getProduct();
    ...
    public function wishlistItemsRenewed()
    {
        ...
        if (method_exists($_wishlistHelper, 'getWishlistItemCollection')) {
            ...
            if (method_exists($_wishlistHelper, 'getItemCollection')) {
        ...
        Mage::helper('catalogpermissions/connection')->addDisabledAttrToFilter(
            $collection, 'product_id', array('noPriceFilter' => true)
        );
    ...
    public static function salableAfter($event)
    {
        ...
        if (Mage::helper('catalogpermissions')->checkVisibility($product, false)) {
            $salable = $event->getSalable();
            $salable->setIsSalable(false);
            $salable->setIsInStock(false);
        }
    }
    ...
    protected function _checkCategoryViewBlock(Mage_Catalog_Block_Category_View $block)
    {
        ...
        if ($this->_getConnectionHelper()->isCategoryDisabled($categoryId)) {
            $block->getLayout()->getMessagesBlock()->addNotice(
    ...
    public function forceCheckSalableAndFlushMinPrice($observer)
    {
        /** @var Mage_Catalog_Model_Product $product */
        $product = $observer['product'];
        $realSalable = $this->_forceCheckSalable($product);
        if (!is_null($realSalable)) {
            $product->setIsSalable($realSalable);
            $product->getStockItem()->setIsInStock($realSalable);
            $product->unsMinPrice();
        }
        return $this;
    }

    public function forceCheckSalableAndFlushMinPriceInCollection($observer)
    {
        /** @var Mage_Catalog_Model_Product $product */
    ...
    public function forceCheckSalable($observer)
    {
        /** @var Mage_Catalog_Model_Product $product */
        $product = $observer['product'];
        /** @var Varien_Object $salableObject */
        $salableObject = $observer['salable'];
        $realSalable = $this->_forceCheckSalable($product);
        if (!is_null($realSalable)) {
            $salableObject->setIsSalable($realSalable);
            $salableObject->setIsInStock($realSalable);
        }
        return $this;
    }

    /**
     * Force check all subproducts to make sure they are not disabled by the extension.
     * Returns null if the product is not bundled.
     *
     * @param Mage_Catalog_Model_Product $product
     * @return bool
     */
    private function _forceCheckSalable($product)
    {
        if (
            $product == null
        ...
        if (is_null($product->getData('is_salable'))) {
            return null;
        }
        ...
        $selectionCollection = $type->getSelectionsCollection($optionCollection->getAllIds(), $product);
