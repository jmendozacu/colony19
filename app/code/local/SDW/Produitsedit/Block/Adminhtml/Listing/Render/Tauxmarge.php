...
class SDW_Produitsedit_Block_Adminhtml_Listing_Render_Tauxmarge extends Mage_Adminhtml_Block_Widget_Grid_Column_Renderer_Abstract
{
    public function render(Varien_Object $row)
    {
		...
		if($price == 0) return '<span id="taux_marge_'.$row->getId().'" data-price='.$price.'></span>';	
		...
		return '<span id="taux_marge_'.$row->getId().'" data-price='.$price.'>'.number_format((($price - $prix_revient)/ $price) * 100, 1,',',''). '%</span>';
    }
}