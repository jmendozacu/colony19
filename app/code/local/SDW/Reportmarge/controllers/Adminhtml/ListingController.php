<?php
class SDW_Reportmarge_Adminhtml_ListingController extends Mage_Adminhtml_Controller_Action
{
	...
	public function printorderAction()
	{
		...
		if ($providerId!='' && $order_id!='') {
			...
			$pdf = Mage::getModel('provider/pdf_provider')->getPdf($providerOrder,$order_id);	
	...
	{
		$idReportmarge = (int) $this->getRequest()->getPost('idReportmarge');
		...
		// now $write is an instance of Zend_Db_Adapter_Abstract
		$write->query("UPDATE `mgt_sdw_provider_order` SET status = '".$value."' WHERE id_provider=".$idReportmarge." AND provider_order_id=".$providerOrderId);
	}
}
