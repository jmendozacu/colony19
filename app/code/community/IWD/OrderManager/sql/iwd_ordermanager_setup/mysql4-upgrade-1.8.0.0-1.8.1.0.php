...
$installer->run("
    ...
    CREATE TABLE {$this->getTable('iwd_cataloginventory_stock_address')}(
        ...
        CONSTRAINT `FK_IWD_STOCK_ADDR_STOCK_ID_CATINV_STOCK_STOCK_ID` FOREIGN KEY (`stock_id`) REFERENCES {$this->getTable('cataloginventory_stock')} (`stock_id`) ON DELETE CASCADE ON UPDATE CASCADE
...
$installer->run("
    ...
    CREATE TABLE {$this->getTable('iwd_cataloginventory_stock_store')}(
        ...
        CONSTRAINT `FK_IWD_STOCK_STORE_STOCK_ID_CATINV_STOCK_STOCK_ID` FOREIGN KEY (`stock_id`) REFERENCES {$this->getTable('cataloginventory_stock')} (`stock_id`) ON DELETE CASCADE ON UPDATE CASCADE,
        CONSTRAINT `FK_IWD_STOCK_STORE_STORE_ID_CORE_STORE_STORE_ID` FOREIGN KEY (`store_id`) REFERENCES {$this->getTable('core_store')} (`store_id`) ON DELETE CASCADE ON UPDATE CASCADE
...
$installer->run("
    ...
    CREATE TABLE {$this->getTable('iwd_cataloginventory_stock_order_item')}(
        ...
        CONSTRAINT `FK_IWD_ORDER_ITEM_STORE_STOCK_ID_CATINV_STOCK_STOCK_ID` FOREIGN KEY (`stock_id`) REFERENCES {$this->getTable('cataloginventory_stock')} (`stock_id`) ON DELETE CASCADE ON UPDATE CASCADE,
        CONSTRAINT `FK_IWD_ORDER_ITEM_STORE_STORE_ID_ORDER_ITEM_ID` FOREIGN KEY (`order_item_id`) REFERENCES {$this->getTable('sales_flat_order_item')} (`item_id`) ON DELETE CASCADE ON UPDATE CASCADE
...
$installer->run("
    ...
    CREATE TABLE {$this->getTable('iwd_cataloginventory_stock_order')}(
        ...
        `qty_ordered` decimal(12,4) DEFAULT '0.0000' COMMENT 'Ordered Items Qty (without virtual products, canceled and refunded items)',
        `assigned` smallint(2) DEFAULT '0' COMMENT 'Is Order Assigned To Stock',
        PRIMARY KEY (`order_id`),
        CONSTRAINT `FK_IWD_ORDER_STORE_STORE_ID_ORDER_ID` FOREIGN KEY (`order_id`) REFERENCES {$this->getTable('sales_flat_order')} (`entity_id`) ON DELETE CASCADE ON UPDATE CASCADE
