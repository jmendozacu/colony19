        ...
        foreach ($data as $key => $obj) {
            try {
                ...
                if (!isset($obj->shipping[0])) {
                    throw new Exception('Invalid shipping params');
                }
    ...
    protected function _prepareShippingParams($orderId, $shipping, &$message)
    {
        ...
        if (isset($rateGroups[$code][$rateName])) {
            ...
            if (empty($shipping->amount_excl_tax) || empty($shipping->amount_incl_tax)) {
                ...
                $params['s_amount_excl_tax'][$rateName] = Mage::helper('tax')->getShippingPrice($rate->getPrice(), Mage::helper('tax')->displayShippingPriceIncludingTax(), $order->getShippingAddress(), null, $store);
                $params['s_amount_incl_tax'][$rateName] = Mage::helper('tax')->getShippingPrice($rate->getPrice(), true, $order->getShippingAddress(), null, $store);
            }
        }
