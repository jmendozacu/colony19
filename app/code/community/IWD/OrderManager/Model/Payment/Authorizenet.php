...
class IWD_OrderManager_Model_Payment_Authorizenet extends IWD_OrderManager_Model_Payment_Authorizenet_Rewrite
{
    ...
    /* * * * * CUSTOM * * * */
    public function fetchTrxInfo($txnId)
    {
        $txn = null;

        try {
            $txn = Mage::getModel('sales/order_payment_transaction')->load($txnId);
            ...
            IWD_OrderManager_Model_Logger::log($e->getMessage(), 'Unable to fetch transaction details.');
        }
        Mage::unregister('current_transaction');
        return $txn;
    }

    public function refundAuthorizeNet($order, $trxId, $amount)
    {
        try {
    ...
    public function voidAuthorizeNetTransaction($order, $trxId)
    {
        try {
            $payment = $order->getPayment();
            $payment = $this->restorePaymentData($payment);
